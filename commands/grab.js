...
run: async (client, message, args, { GuildDB }) => {
  ...
  .addField(`▶ Canción:`, `\`${GuildDB ? GuildDB.prefix : client.botconfig.DefaultPrefix
  }play ${player.queue.current.uri}\``)
  .addField(`🔎 Guardado en:`, `<#${message.channel.id}>`)
  .setFooter(`Solicitado por: ${player.queue.current.requester.tag}`, player.queue.current.requester.displayAvatarURL({
    dynamic: true
  }))
    ).catch(e=>{
    ...
    client.sendTime(message.channel, "✅ | **¡Revisa tu DM!**")
  },
  SlashCommand: {
/**
*
...
*/
  run: async (client, interaction, args, { GuildDB }) => {
    ...
    if (!player) return client.sendTime(interaction, "❌ | **No está sonando nada en este momento...**");
    ...
    client.sendTime(interaction, "✅ | **¡Revisa tu DM!**")
  },
  },
};
