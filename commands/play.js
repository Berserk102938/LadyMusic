...
module.exports = {
    ...
    run: async (client, message, args, { GuildDB }) => {
        if (!message.member.voice.channel) return client.sendTime(message.channel, "❌ | **¡Debes estar en un canal de voz para reproducir algo! **");
        ...
        if (!SearchString) return client.sendTime(message.channel, `**Usa - **\`${GuildDB.prefix}play [link de la cancion]\``);
       ...
       return client.sendTime(message.channel,"❌ | **Lavalink node not connected**");
        ...
        if (!player) return client.sendTime(message.channel, "❌ | ** No se está reproduciendo nada en este momento ... **");
        ...
        try {
            if (SearchString.match(client.Lavasfy.spotifyPattern)) {
                ...
                if (Searched.loadType === "PLAYLIST_LOADED") {
                    ...
                    SongAddedEmbed.setAuthor(`Lista de reproducción agregada a la cola`, message.author.displayAvatarURL());
                    SongAddedEmbed.addField("En cola", `\`${Searched.tracks.length}\` songs`, false);
                    ...
                    if (player.queue.totalSize > 1) SongAddedEmbed.addField("Posición en fila", `${player.queue.size - 0}`, true);
                    Searching.edit(SongAddedEmbed);
                } else {
                    return client.sendTime(message.channel, "**No se encontraron coincidencias para - **" + SearchString);
                }
            } else {
                let Searched = await player.search(SearchString, message.author);
                if (!player) return client.sendTime(message.channel, "❌ | **Nada está sonando en este momento ...**");

                if (Searched.loadType === "NO_MATCHES") return client.sendTime(message.channel, "**No se encontraron coincidencias para - **" + SearchString);
                else if (Searched.loadType == "PLAYLIST_LOADED") {
                    player.queue.add(Searched.tracks);
                    if (!player.playing && !player.paused && player.queue.totalSize === Searched.tracks.length) player.play();
                    SongAddedEmbed.setAuthor(`Lista de reproducción agregada a la cola`, client.botconfig.IconURL);
                    SongAddedEmbed.setThumbnail(Searched.tracks[0].displayThumbnail());
                    SongAddedEmbed.setDescription(`[${Searched.playlist.name}](${SearchString})`);
                    SongAddedEmbed.addField("En cola", `\`${Searched.tracks.length}\` songs`, false);
                    SongAddedEmbed.addField("Duración de la lista de reproducción", `\`${prettyMilliseconds(Searched.playlist.duration, { colonNotation: true })}\``, false);
                    Searching.edit(SongAddedEmbed);
                } else {
            ...
            if (search.match(client.Lavasfy.spotifyPattern)) {
                ...
                switch (Searched.loadType) {
                    case "LOAD_FAILED":
                        ...
                        return client.sendError(interaction, `❌ | **Hubo un error al buscar **`);

                    case "NO_MATCHES":
                        if (!player.queue.current) player.destroy();
                        return client.sendTime(interaction, "❌ | **No se encontraron resultados.**");
                    case "TRACK_LOADED":
                    ...
                    case "SEARCH_RESULT":
                        ...
                        let SongAdded = new MessageEmbed();
                            ...
                            if (player.queue.totalSize > 1) SongAdded.addField("Posición en fila", `${player.queue.size - 0}`, true);
                    ...
                    case "PLAYLIST_LOADED":
                        ...
                        Playlist.setAuthor(`Lista de reproducción agregada a la cola`, client.botconfig.IconURL);
                        Playlist.setDescription(`[${Searched.playlistInfo.name}](${interaction.data.options[0].value})`);
                        Playlist.addField("En cola", `\`${Searched.tracks.length}\` songs`, false);
                ...
                try {
                    ...
                    if (res.loadType === "LOAD_FAILED") {
                        if (!player.queue.current) player.destroy();
                        return client.sendError(interaction, `:x: | ** Hubo un error al buscar**`);
                    }
                } catch (err) {
                    return client.sendError(interaction, `Hubo un error durante la búsqueda: ${err.message}`);
                }
                switch (res.loadType) {
                    case "NO_MATCHES":
                        if (!player.queue.current) player.destroy();
                    ...
                    case "TRACK_LOADED":
                        ...
                        let SongAddedEmbed = new MessageEmbed();
                            ...
                            SongAddedEmbed.addField("Duración", `\`${prettyMilliseconds(res.tracks[0].duration, { colonNotation: true })}\``, true);
                            if (player.queue.totalSize > 1) SongAddedEmbed.addField("Posición en fila", `${player.queue.size - 0}`, true);
                            return interaction.send(SongAddedEmbed);
                    ...
                    case "PLAYLIST_LOADED":
                        ...
                        await player.play();
                        let SongAdded = new MessageEmbed();
                        SongAdded.setAuthor(`Lista de reproducción agregada a la cola`, client.botconfig.IconURL);
                        ...
                        SongAdded.addField("En cola", `\`${res.tracks.length}\` songs`, false);
                        SongAdded.addField("Duración de la lista de reproducción", `\`${prettyMilliseconds(res.playlist.duration, { colonNotation: true })}\``, false);
                        return interaction.send(SongAdded);
                    case "SEARCH_RESULT":
                        ...
                        if (!player.playing && !player.paused && !player.queue.length) {
                            ...
                            SongAddedEmbed.addField("Duración", `\`${prettyMilliseconds(track.duration, { colonNotation: true })}\``, true);
                            if (player.queue.totalSize > 1) SongAddedEmbed.addField("Posición en fila", `${player.queue.size - 0}`, true);
                            player.play();
                            return interaction.send(SongAddedEmbed);
