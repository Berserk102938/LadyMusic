...
module.exports = {
  ...
  run: async (client, message, args, { GuildDB }) => {
    ...
    if (!args[0] && !player) return client.sendTime(message.channel, "❌ | ** No se está reproduciendo nada en este momento ...**");
    ...
    if (!lyrics) return client.sendTime(message.channel, `**No se encontraron letras para -** \`${SongTitle}\``);
    lyrics = lyrics.split("\n"); //spliting into lines
  ...
  SlashCommand: {
    ...
    run: async (client, interaction, args, { GuildDB }) => {
      ...
      if (!interaction.data.options && !player) return client.sendTime(interaction, "❌ | **Nada está sonando en este momento...**");
      ...
      if (lyrics.length === 0)
        return client.sendTime(interaction, `**No se encontraron letras para -** \`${SongTitle}\``);
      lyrics = lyrics.split("\n"); //spliting into lines
      let SplitedLyrics = _.chunk(lyrics, 40); //45 lines each page
