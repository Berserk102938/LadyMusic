...
module.exports = {
    ...
    run: async (client, message, args, { GuildDB }) => {
        ...
        if (player.paused) return client.sendTime(message.channel, "❌ | **¡la música ya está en pausa! **");
        player.pause(true);
        let embed = new MessageEmbed().setAuthor(`Pausada!`, client.botconfig.IconURL).setColor(client.botconfig.EmbedColor).setDescription(`Type \`${GuildDB.prefix}resume\` para seguir escuchando!`);
     ...
     */
        run: async (client, interaction, args, { GuildDB }) => {
            ...
            if (player.paused) return client.sendTime(interaction, "¡La música ya está en pausa!");
            player.pause(true);
